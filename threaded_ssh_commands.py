...
def get_netbox_hosts():
    ...
    netbox_api_object = pynetbox.api(
        url="https://netbox.widgets.com", token=os.environ["NETBOX_API_TOKEN"]
    )
    ...
    netbox_hosts = netbox_api_object.dcim.devices.filter(
        tag="devices_of_interest", site="widgets_inc_data_center"
    )
...
def execute_command_on_active_list(active_hosts_list):
    ...
    for host in active_hosts_list:
        thread_objects_list.append(
            threading.Thread(
                target=execute_command_on_host, name=f"<{host} thread>", args=(host,)
            )
        )
...
def create_list_of_active_hosts(netbox_host_stings_list):
    ...
    for host in netbox_host_stings_list:
        thread_objects_list.append(
            threading.Thread(target=check_host_active_status, args=(host,))
        )
